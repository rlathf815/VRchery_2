Shader "MagicCirclesFX/MaskBlended"
{
	Properties
	{
		[HDR]_TintColor("Color", Color) = (1,1,1,1)
		_MainTex ("Texture", 2D) = "white" {}
		_XOffsetTime("TimeOffsetX", Float) = 0
		_YOffsetTime("TimeOffsetY", Float) = 0
		[Space]
		[Space]
		[Space]
	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 border = 0;
				...
				float4 col = tex2D(_MainTex, pixelInput.uv+_Time.y*float2(_XOffsetTime,_YOffsetTime))*_TintColor*pixelInput.color;
				col.rgb += border;

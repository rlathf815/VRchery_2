Shader "MagicCirclesFX/MaskBlendedNoZ"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata vertexInput)
			{
				...
				pixelInput.normal = UnityObjectToWorldNormal(vertexInput.normal);
				pixelInput.viewDir = mul(unity_ObjectToWorld, vertexInput.vertex) - _WorldSpaceCameraPos;
				return pixelInput;
			}
			...
			float4 frag (v2f pixelInput) : SV_Target
			{
				...
				float fresnel = 1 - dot(normalize(pixelInput.viewDir),normalize(pixelInput.normal));
				fresnel = pow(fresnel, -1*_FrenelStr);
